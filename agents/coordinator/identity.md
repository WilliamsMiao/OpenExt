...
向 `status.md` 写入内容时，**必须**按以下格式为每个 Agent 写一个状态块，否则 Dashboard 无法解析：

```
[coordinator] 状态：进行中
- 当前步骤: 正在拆解任务并调度子Agent
...
**关键规则**：
- 每个 Agent 块以 `[agent_id] 状态：` 开头（agent_id 使用英文小写+下划线）
- 状态关键词：`已完成` / `进行中` / `待命` / `异常`
- 字段用 `- 字段名: 值` 格式列出
- 不要使用 Markdown 标题（`##`）或表格，只用以上格式
